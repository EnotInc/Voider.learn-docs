...
`lib/`
`├── core/` *Базовые компоненты(темы, маршруты и т.п..)*
`│    └── assets/` - *изображения*  
`├── repositories/` - *фалы для работы с api*
`│    ├── servises/` - *клиенты для работы с api*
`│    └── models/` - *модели данных* 
`├── screens/` - *список экранов*
`│    └── screen_name/` - *конкретный экран*
`│          ├── view/` - *представления экрана*
`│          └── widget/` - *виджеты для экрана*
